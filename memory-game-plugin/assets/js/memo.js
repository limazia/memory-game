...
//				location = location.split('/').splice(location.length - 1, 1, 'images');
	...
	*/
	MemoryGame.prototype.markup = function() {
		...
		this.header.innerHTML += 
		`<div class="mg-points">
			Pontos: <span id="mg-points" class="mg-text--md">000</span>
		</div>`;
		...
		this.start.textContent = 'Começar';
	...
	*/
	MemoryGame.prototype.events = function() {
		...
		document.getElementsByClassName('mg-start')[0].addEventListener('click', function() {
			...
			hurryUp.textContent = 'Se apresse!';
		...
		switch(type) {
			case 'success':
				content = `
					...
					<p>Você terminou o jogo em:<br/>
					<span id="mg-final-time" class="mg-special"></span>
					</p>
					<p>pontos: <span id="mg-final-points" class="mg-special">${document.getElementById('mg-points').innerHTML}</span><br/>
					bônus de tempo: <span id="mg-final-bonus" class="mg-special">${(parseInt(document.getElementById('mg-minutes').innerHTML)*60 + parseInt(document.getElementById('mg-seconds').innerHTML))*10}</span><br/>
					total: <span id="mg-final-total" class="mg-special"></span>
					</p>`;
				contentFooter = `<button class="mg-reset mg-btn mg-btn--lg mg-btn--primary">Resolva outro</button>` ;
				break;
			...
			case 'fail':
				...
				contentFooter = `
					<button class="mg-reset mg-btn mg-btn--sm mg-btn--primary">Tente novamente!</button>` ;
				break;
			...
			case 'reset':
				...
				contentFooter = `
					<button class="mg-reset mg-btn mg-btn--lg mg-btn--secondary">Sim</button>
					<button class="mg-btn mg-continue mg-btn--lg mg-btn--primary">Cancelar</button>
					`;
				break;
			...
			default:
				content = `
					<h3>Oops!</h3>
					<p>Não previmos esta situação, feche a caixa de diálogo ou comece novamente</p>`;
				contentFooter = `
					<button class="mg-reset mg-btn--sm mg-btn--seconady">Redefinir</button>`;
		}
